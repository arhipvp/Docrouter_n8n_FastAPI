...
1. **Запуск сервисов.** На локальной машине поднимается FastAPI-сервис (`uvicorn app:app --host 127.0.0.1 --port 8081 --reload`). Параллельно пользователь запускает workflow `My workflow 3` в n8n (Manual Trigger).
2. **Получение файла.** Нода `Read/Write Files from Disk` выбирает первый PDF из `C:/Data/inbox`, после чего `Loop Over Items` гарантирует поштучную обработку.
3. **Подготовка контекста.** Скрипт `Code in JavaScript` фиксирует имя и абсолютный путь файла в `item.json` (`inbox_name`, `inbox_path`).
4. **Извлечение текста.** Нода `Text` вызывает FastAPI `/extract-text-by-path`; при пустом тексте сервис автоматически выполняет OCR через OCRmyPDF. Ответ содержит текст, количество страниц, размер и флаги OCR.
5. **Определение языка.** Нода `Lang` обращается к `/lang` и получает язык и вероятность. Результат используется дальше в workflow и в метаданных.
6. **Конструирование сайдкаров.** Нода `Подготовка сайдкаров` агрегирует данные (`Text`, `Lang`, `Code in JavaScript`), формирует черновые summary RU/DE, подготавливает тексты для `content.ru.txt`/`content.de.txt`, создаёт базовую структуру `metadata` и ограничивает исходный текст для передачи на бэкенд.
7. **Получение каталога архива.** Нода `Folder Endpoints` берёт все конечные пути из `/folder-endpoints` для подстановки в промпт маршрутизатора.
8. **Маршрутизация.** Узел `AI Agent` с моделью OpenRouter (`openai/gpt-4o`) анализирует текст и выбирает существующую папку или предлагает новую (JSON). При необходимости новый путь создаётся через `/fs-mkdir`.
9. **Перемещение и финализация.** Нода `Перемещаем файл при готовой папке` вызывает `/fs-move`, передавая путь источника, финальный каталог/имя, а также подготовленные `metadata`, `summaries`, `content`, `content_truncated` и `source_text`. FastAPI переносит PDF и сразу создаёт рядом `*.metadata.json`, `*.content.ru.txt`, `*.content.de.txt` с учётом переданных данных (или генерирует заглушки).
10. **Завершение цикла.** Workflow возвращается в `Loop Over Items`, чтобы перейти к следующему документу (если он есть). Финальный отчёт печатается через `/print-report` вручную либо в расширенной версии пайплайна.
